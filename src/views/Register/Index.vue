    ...
    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
      <div class="login">
        <el-card>
          ...
          <el-form ref="form" class="login-form" :model="model" @submit.prevent="register">
            ...
            <el-form-item prop="username">
              ...
              <div v-if="validationErrs['username']" class="text-danger">
                {{ validationErrs['username'] }}
              </div>
            </el-form-item>
            <div style="display: flex; align-items: center; justify-content: space-between; width: 100%">
              <el-form-item prop="firstName" style="width: 48%">
                <el-input v-model="model.firstName" placeholder="First Name"></el-input>
                <div v-if="validationErrs['firstName']" class="text-danger">
                  {{ validationErrs['firstName'] }}
                </div>
              </el-form-item>

              <el-form-item prop="lastName" style="width: 48%">
                <el-input v-model="model.lastName" placeholder="Last Name"></el-input>
                <div v-if="validationErrs['lastName']" class="text-danger">
            ...
            <el-form-item prop="confirmPassword">
              ...
              ></el-input>
              <div v-if="validationErrs['confirmPassword']" class="text-danger">
                {{ validationErrs['confirmPassword'] }}
              </div>
            </el-form-item>
            <el-form-item>
...
export default defineComponent({
  ...
  // },
  setup() {
    const model = ref({ email: '', username: '', firstName: '', lastName: '', password: '', confirmPassword: '' })
    ...
    const register = async () => {
      try {
        ...
        if (isEmpty(validationErrs.value)) {
          loading.value = true
          const { user }: any = await createUserWithEmailAndPassword(getAuth(), model.value.email, model.value.password)
          if (user) {
            await writeUserData(user.uid, model.value.username, model.value.firstName, model.value.lastName, model.value.email)
            setToken(user.accessToken)
            loading.value = false
            Router.push('/')
          }
    ...
    return { model, register, loading, validationErrs, handlePopupLogin }
  }
})
</script>
